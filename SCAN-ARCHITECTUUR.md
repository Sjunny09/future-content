...
- `Lead`: `email` (uniek), naam, telefoon, `geanonimiseerd` (18 maanden). `Answer`: één rij per beantwoorde vraag (`vraagId`, `vraagTitel`, `waarde` Json), cascade delete op `ScanJob`. `LoomVideo`: 24u-opvolging. `Payment`: Mollie/Stripe pilot.
- Migraties: `20260421051833_init_scan`, `20260629104500_diepte_scan`, `20260703160000_add_opmerking_scan`.
...
3. Scrape is de wildcard: normaal 2-8s, maar een trage site kan de 12s Jina-timeout plus tot 10s Cheerio opvreten.
