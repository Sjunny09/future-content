...
- **Storing van 20 september 2026, en wat er sindsdien tegen beschermt.** De bridge had vanaf dag één nooit één lead afgeleverd: het `OS_WEBHOOK_SECRET` in de Vercel-productie van deze repo week af van het secret dat het live OS accepteert, dus elke POST kreeg een 401. Niemand zag dat, want `notifyOs` is fail-soft en logt alleen naar de Vercel-logs. Vier dingen zijn erbij gezet:
  - `notifyOs` stuurt bij een niet-ok antwoord of een onbereikbaar OS een **Telegram-melding** (`lib/scan/telegramPing.ts`, losgetrokken uit `notifyTelegram`). Telegram is het enige kanaal dat aantoonbaar bij John aankomt.
  - De nachtelijke cron (`app/api/cron/loom-herinnering/route.ts`, functie `herhaalOsBridge`) biedt afgeronde scans van de laatste zeven dagen **opnieuw** aan. De OS-route dedupt op `scanLeadId`, dus dit is idempotent en heelt een korte storing vanzelf. Let op de ondergrens `HERHAAL_VANAF`: leads van vóór 21-09-2026 staan met de hand in het OS zonder `scanLeadId`, opnieuw aanbieden zou daar een dubbele relatie maken.
  - **`GET /api/scan/bridge-check`** meet het pad zoals productie het loopt (zelfde env, zelfde URL, zelfde secret) en schrijft niets: hij stuurt expres een onleesbare body, dus 400 betekent gezond, 401 een fout secret en geen antwoord een foute `OS_BASE_URL`. Publiek, gedempt op één echte meting per minuut. Controleer hiermee na elke env- of deploywijziging, in plaats van te wachten op een echte lead.
  - `notifyOs` slaat testscans over (`job.isTest`), zodat John kan testen zonder relaties in het echte CRM te maken. Een testscan stuurt wél de notificatiemail, dus dat blijft de manier om de mailroute na te meten.
- **Zelfde dag, zelfde soort fout: de notificatiemail.** Die ging naar `john@future-content.nl`, een ZXCS-postbus die John niet leest, terwijl de MailLog netjes "verstuurd" zei (dat zegt alleen dat Resend hem aannam). `SCAN_NOTIFY_EMAIL` staat nu op `hello@future-content.nl`.
- **Achterhaald (gemeten 20-09-2026):** het juli-gat hieronder klopt niet meer voor de verzendkant. `notifyOs` stuurt `opmerking` en `diagnose` wél mee; het OS bewaart ze onder `_opmerking` en `_diagnose` in `scanAntwoorden`. Wat blijft staan is de leads-UI: die is video-first, niet bel-first.
