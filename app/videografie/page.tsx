...
export const metadata: Metadata = {
  title: "Videografie · Future Content",
  description:
  ...
  return (
    <main className="bg-[#FAFAF8]">
      <section className="max-w-6xl mx-auto px-6 pt-32 pb-14 md:pt-40">
        ...
        <h1
          className="text-4xl md:text-[3.2rem] font-bold text-[#1A1A18] mt-4 leading-[1.05] max-w-2xl tracking-[-0.01em]"
          style={{ fontFamily: "var(--font-playfair)" }}
        >
          Videografie die mensen laat stilstaan.
        </h1>
        <p className="text-[#6B7280] text-lg mt-5 max-w-xl leading-relaxed">
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          {VIDEO_LINKS.map((v, i) => (
            <Link
              key={v.href}
              href={v.href}
              className="group relative overflow-hidden rounded-2xl bg-[#0F0F0D] min-h-[280px] flex flex-col justify-end p-8"
            >
              <Image
              ...
              <div className="relative z-10">
                <h2
                  className="text-2xl font-bold text-[#FAFAF8] mb-1.5"
                  style={{ fontFamily: "var(--font-playfair)" }}
                >
                  {v.label}
                </h2>
                <p className="text-[#FAFAF8]/70 text-sm mb-4">{v.desc}</p>
