    ...
    <main className="bg-[#FAFAF8]">
      ...
      <section className="max-w-5xl mx-auto px-6 pt-32 pb-16 md:pt-40">
        <motion.div variants={fadeUp} initial="hidden" animate="show" className="max-w-3xl">
          ...
          <h1
            className="text-4xl md:text-[3.4rem] font-bold text-[#1A1A18] mt-4 mb-6 leading-[1.05] tracking-[-0.01em]"
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {BRANCHES.sort((a, b) => a.order - b.order).map((b, i) => (
              ...
              >
                <Link
                  href={`/voor/${b.slug}`}
                  className="block bg-[#FAFAF8] hover:bg-white border border-[#E5E0D8] hover:border-[#C9A96E]/40 rounded-2xl p-7 transition-all h-full"
                >
                  <span className="text-[10px] font-semibold uppercase tracking-[0.15em] text-[#C9A96E]">
                    {b.shortName}
                  </span>
                  <h3
                    className="text-xl md:text-2xl font-bold text-[#1A1A18] mt-2 mb-3 leading-tight"
                    style={{ fontFamily: "var(--font-playfair)" }}
                  >
                    {b.skinTitel}
                  </h3>
                  <p className="text-[#6B7280] text-sm leading-relaxed mb-5">{b.heroLead}</p>
